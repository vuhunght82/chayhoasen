...
import { Branch, Category, MenuItem, Order, OrderStatus, PaymentMethod, PrinterSettings, KitchenSettings, SavedSound, Topping, ToppingGroup, CustomerSettings, PrintStation } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(ref(database), (snapshot) => {
      ...
      if (data) {
        ...
        setPrintStations(firebaseListToArray(data.printStations));
      }
      setLoading(false);
    });
  ...
  const handleSetPrintStations = (newStations: PrintStation[] | ((prev: PrintStation[]) => PrintStation[])) => {
      const dataToSet = typeof newStations === 'function' ? newStations(printStations) : newStations;
      set(ref(database, 'printStations'), dataToSet);
  };
    ...
    switch (currentView) {
        ...
        case 'admin':
            return <AdminView 
                        ...
                        setPrintStations={handleSetPrintStations}
